...
local function LoadConfig(name)
    ...
    return GetModConfigData(name, mod)
        or GetModConfigData(name, KnownModIndex:GetModActualName(mod))
end
...
local function GetFoodsAndTimers(self)
    if true then
        ...
        for name, identifier in pairs((self and self.foodtile_identifiers) or temp_foodtile_identifiers) do
            local data = (self and self.foodtiles and self.foodtiles[identifier])
                or temp_foodtiles[identifier]
            local food_name = data.name
            local food_time = (self and self.GetTimerForFood and self:GetTimerForFood(food_name))
                or temp_timers[food_name]
        ...
        for name, identifier in pairs((self and self.spices) or temp_spice_identifiers) do
            if identifier.name and identifier.name ~= "undefined" then
                ...
                local time = (self and self.GetTimerForBuff and self:GetTimerForBuff(name))
                    or temp_spice_timers[name]
                if time > 0 then
...
local function SavePersistentData(self) -- Great thing about the class this time is that it has all its data in one place and isn't spread around a few entities.
    ...
    if seed then
        ...
        local data = {}
        TheSim:GetPersistentString(id, function(success, _data)
            if success then
...
local FoodMemory = Class(Widget, function(self, owner)
    ...
    self.togglebutton = self:AddChild(
        ImageButton("images/frontend.xml", "button_square_halfshadow.tex")
    )
    self.ontogglebuttonfn = function()
        self:ToggleMemoryDisplay()
    end
    ...
    self.togglebutton_icon = self.togglebutton:AddChild(
        Image("images/inventoryimages1.xml", "meatballs.tex")
    )
    self.togglebutton_icon:SetPosition(-7, 1)
    self.togglebutton_icon:SetScale(0.5, 0.5, 0.5)

    self.togglebutton_icon_alt = self.togglebutton:AddChild(
        Image("images/inventoryimages1.xml", "deerclops_eyeball.tex")
    )
    self.togglebutton_icon_alt:SetPosition(-7, 1)
    self.togglebutton_icon_alt:SetScale(0.5, 0.5, 0.5)
    self.togglebutton_icon_alt:Hide()

    self.memorydisplay_bg = self:AddChild(
        Image("images/global.xml", "square.tex", "square.tex", "square.tex")
    )
    self.memorydisplay_bg:SetTint(0, 0, 0, 0.8)
    ...
    self:Show()
    self.oneatfoodfn = function(src, data)
        self:AddFoodTimer(data)
    end
    owner:ListenForEvent("MOD_WarlyMemory_successful_eat", self.oneatfoodfn)
    self.ontimerdonefn = function(src, data)
        if string.sub(data.name, -7, -1) == "_memory" then
            self:RemoveFoodTimer(string.sub(data.name, 1, -8))
        end
    end --Probably our timer.
    owner:ListenForEvent("timerdone", self.ontimerdonefn)
    self.onsortfn = function(src, fn)
        self:SortFoodTilesByFunction(fn)
    end
...
local function GetFormatedBuff(buff, temperature_duration, foodname) -- Return a table which tells me the name and duration.
    ...
    local buff_time = temperature_duration
        or TUNING[string.upper(buff) .. "_DURATION"]
        or special_buff_durations[buff]
    if foodname then
        buff = foodname .. "_buff"
    end
    ...
    if data.hunger or data.sanity or data.health then --I don't care about foodtype anymore because food triggers are handled in modmain.lua. I only care if the food is actually a food(And it should have hunger,sanity,health then)
        ...
        if data.temperature then
            table.insert(
                buffs,
                #buffs + 1,
                GetFormatedBuff(data.temperature, data.temperatureduration, foodname)
            )
        end
        ...
        return {
            ...
            stats = { hunger = data.hunger, sanity = data.sanity, health = data.health },
            buffs = buffs,
        }
    else
        return nil
...
function FoodMemory:StartTimerComponent(foodname, time)
    ...
    if not ThePlayer.components.timer then
        print("Error, Player has no timer component")
        return nil
    end
...
function FoodMemory:StartTimerComponentForBuff(buff, time)
    ...
    if not ThePlayer.components.timer then
        print("Error, Player has no timer component, buff will not be timed")
        return nil
    end
    local timer = ThePlayer.components.timer
...
function FoodMemory:GetTimerForFood(foodname, time_format)
    ...
    elseif time_format == "min" then
        ...
        local s = string.format("%.0f", time_left % 60) ~= "60"
                and string.format("%.0f", time_left % 60)
            or "0"
...
function FoodMemory:AssessWidgetInfo()
    for k, data in pairs(self.foodtiles) do
        ...
        if self.foodtile_identifiers[name] == k then
            ...
            print(
                "BAD MATCH self.foodtile_identifiers:",
                self.foodtile_identifiers[name],
                "self.foodtiles:",
                k
            )
            return false
...
function FoodMemory:AddFoodTimer(foodname, time, nosound, value)
    ...
    if dish then
        ...
        local obj = self.memorydisplay_bg:AddChild(
            Image("images/frontend.xml", "button_square_halfshadow.tex")
        )
        table.insert(
            self.foodtiles,
            #self.foodtiles + 1,
            { widget = obj, row = self.rows, column = self.rowitems, name = "undefined" }
        )
        ...
        for k, statname in ipairs(stats) do
            local tex = dish.stats[statname]
                    and dish.stats[statname] >= 0
                    and "status_" .. statname .. ".tex"
                or (dish.stats[statname] and "status_" .. statname .. "_bad.tex")
                or "coin_unknown.tex"
            local atlas = tex ~= "coin_unknown.tex" and "images/global_redux.xml"
                or "images/quagmire_recipebook.xml"
        ...
        local on_gainfocus_fn = function()
            food.text:Show()
            food.num_eaten:Hide()
            food.timer:Hide()
            food.bufftimer:Hide()
        ...
        local on_losefocus_fn = function()
            food.text:Hide()
            food.num_eaten:Show()
            food.timer:Show()
            food.bufftimer:Show()
        ...
        food.OnMouseButton = function(_, button, down)
            if button == MOUSEBUTTON_LEFT and down then
                ...
                local s = string.format("%.0f", time % 60) ~= "60"
                        and tonumber(string.format("%.0f", time % 60))
                    or 0
...
function FoodMemory:RemoveFoodTimer(foodname)
    ...
    if not identifier then
        print("Food does not exist on an identifier list, something went wrong!")
        return
    end
...
local sort_fns = {
    shorttime = function(a, b)
        return FoodMemory:GetTimerForFood(a) < FoodMemory:GetTimerForFood(b)
    end,
    longtime = function(a, b)
        return FoodMemory:GetTimerForFood(a) > FoodMemory:GetTimerForFood(b)
    end,
    AZ = function(a, b)
        return STRINGS.NAMES[string.upper(a)] < STRINGS.NAMES[string.upper(b)]
    end,
    ZA = function(a, b)
        return STRINGS.NAMES[string.upper(a)] > STRINGS.NAMES[string.upper(b)]
    end,
    --Should we check the existence of the food property table such that we don't index a nil value?
    hunger_asc = function(a, b)
        return GetFoodPropertyForFood(a).stats.hunger < GetFoodPropertyForFood(b).stats.hunger
    end,
    hunger_desc = function(a, b)
        return GetFoodPropertyForFood(a).stats.hunger > GetFoodPropertyForFood(b).stats.hunger
    end,
    health_asc = function(a, b)
        return GetFoodPropertyForFood(a).stats.health < GetFoodPropertyForFood(b).stats.health
    end,
    health_desc = function(a, b)
        return GetFoodPropertyForFood(a).stats.health > GetFoodPropertyForFood(b).stats.health
    end,
}
