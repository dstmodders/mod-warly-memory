...
local function ClearFoodMemory(src)
    ...
    if seed then
        ...
        local data = {}
        _G.TheSim:GetPersistentString(id, function(success, _data)
            if success then
    ...
    inst:DoTaskInTime(0.5, function()
        ...
        if inst == _G.ThePlayer and not (inst.prefab ~= "warly" and warly_only) then
            ...
            if _G.TheWorld.ismastersim then
                _G.ThePlayer:ListenForEvent("oneat", function(src, data)
                    if data.food:HasTag("preparedfood") then
                        _G.ThePlayer:PushEvent("MOD_WarlyMemory_successful_eat", data.food.prefab)
                    end
                end)
            end
...
local onremovefn =
    function(self) --The "onremove" event triggers when a food turns to rot or gets eaten with a stacksize of 1.
...
local function stacksizechange(inst)
    ...
    local d_stack = foodentity
        and foodentity.replica.stackable
        and (
            (d_stacksize - foodentity.replica.stackable:StackSize()) >= 1
            or (d_stacksize - foodentity.replica.stackable:StackSize()) >= 1
        )
...
local function checkfor_stacksizechange(inst) --The problem of "Quick foods may not get registered" is cause by the fact that RPC triggers instantly, unlike the functions that check the food, which means that the food we're checking may get swapped for another one, which just gives us a false value.
    local is_eating = (
            _G.ThePlayer.AnimState:IsCurrentAnimation("eat")
            or _G.ThePlayer.AnimState:IsCurrentAnimation("quick_eat")
            or _G.ThePlayer.AnimState:IsCurrentAnimation("quick_eat_pre")
        )
    local d_stack = (
            foodentity
            and foodentity.replica.stackable
            and (
                ((old_stacksize - foodentity.replica.stackable:StackSize()) == 1)
                or (d_stacksize - foodentity.replica.stackable:StackSize() == 1)
            )
        ) or not foodentity
...
local function OnStartEating(rpc, act, ...)
    if rpc == _G.RPC.LeftClick then
        ...
        if food_ent:HasTag("preparedfood") then
            ...
            old_stacksize = food_ent.replica.stackable and food_ent.replica.stackable:StackSize()
                or 0
            if not food_ent.islisteningforremove then
                food_ent:ListenForEvent("onremove", onremovefn)
                food_ent._islisteningforremove = true
                food_ent:DoTaskInTime(onremoveevent_duration, function()
                    food_ent:RemoveEventCallback("onremove", onremovefn)
                    food_ent.islisteningforremove = nil
                end)
            end
            _G.ThePlayer:DoTaskInTime(frame_error * 3, function()
                d_stacksize = old_stacksize
            end)
...
function _G.c_getfoodstats() --Sort by count; Show percentage.
    ...
    table.sort(sorted_table, function(a, b)
        return food_stats[a] > food_stats[b]
    end)

    for k, food in pairs(sorted_table) do
        local num = _G.tostring(
            _G.tonumber(string.format("%.4f", food_stats[food] * 100 / total_count))
        )
...
function _G.c_setfoodnumberstat(food, num)
    ...
    if food and num then
        ...
        if not food_name then
            print(
                "Error: Food can't be found in strings. Please input the food prefab and not its name"
            )
        else
...
function _G.c_resetfoodstats(t)
    if t == true then
        ...
        print(
            "Note: All food stats will be wiped. This cannot be undone. Type c_resetfoodstats(true) to confirm the data reset."
        )
    end
end
...
local function ConsoleScreenPostInit(self)
    if self.console_edit.prediction_widget then
        local dictionaries = self.console_edit.prediction_widget.word_predictor
                and self.console_edit.prediction_widget.word_predictor.dictionaries
            or {}
